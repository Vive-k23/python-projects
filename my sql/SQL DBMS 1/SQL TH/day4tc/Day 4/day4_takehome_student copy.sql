...
# Question 1:
# 1) Print credit card transactions with sum of transaction_amount on all Fridays and sum of transaction_amount on all other day
select abs(sum(transaction_amount))sum_trans,if(date_format(transaction_date,'%a')='fri','fri','od')as day_of_tran ,account_type from bank_account_details b
join bank_account_transaction bat using(account_number) where Account_type like'%credit%' group by day_of_tran;
# Question 2:
# 2) Show the details of credit cards along with the aggregate transaction amount during holidays  and non holidays. 
select account_type,h.holiday,abs(transaction_amount)transaction_amount, if(transaction_date=h.holiday,'holi','nonholi')as holiday_trans 
from bank_account_details b join bank_account_transaction using(account_number),bank_holidays h 
where Account_type = 'credit card' group by Transaction_amount;
# Question 3:
...
select account_number,transaction_amount,if(transaction_date=h.holiday,'Happy Holiday',transaction_date) as holiday_greet from bank_account_transaction
join bank_holidays h where month(Transaction_Date)='3' group by holiday_greet;



# Question 4:
# 4) Calculate the Bank accrued interest with respect to their RECURRING DEPOSITS  for any deposits older than 30 days . 
select b.account_number, bi.account_type,sum(transaction_amount * interest_rate) as accrued_interest 
from bank_account_details as b join bank_account_transaction as bat on b.Account_Number=bat.Account_Number join bank_interest_rate as bi on b.account_type=bi.account_type  
where b.account_type like '%recurring deposits%' group by account_number;
...
select bat.account_number as credit_card_acc_number,bar.account_type,count(bat.account_number)count_a,Linking_Account_Number as saving_account_number from bank_account_transaction
 as bat join bank_account_relationship_details as bar using(account_number)
where bar.Account_type='credit card' group by Account_Number having count_a>1;
# Question 6:
# 6) Display the Savings Account number whose corresponding AddonCredit card transactions have occured atleast once. 
select bat.account_number,bar.account_type,count(bat.account_number)count_a,Linking_Account_Number from bank_account_transaction
 as bat join bank_account_relationship_details as bar using(account_number)
where bar.Account_type like 'add%' group by Account_Number;
...
select customer_id,length(customer_id) from bank_customer natural join bank_customer_export;
...
select customer_id,customer_name from bank_customer natural join bank_customer_export;
select 
