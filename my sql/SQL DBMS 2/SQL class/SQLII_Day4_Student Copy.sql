...
create view salary_range as select employee_id , first_name , last_name , job_id , salary from employee_details where salary between 30000 and 50000;
...
create view pattern_matching as select employee_id , first_name , last_name , job_id , salary from employee_details where first_name like "%l";
...
create view employee_department as select a.employee_id , first_name , last_name , job_id , salary , DEPARTMENT_ID,DEPARTMENT_NAME , location_id  from employee_details a join department_details b using(department_id);
...
create view stu_adm as select sop , lor from admission_predict where Research = 1;

# Q.13 Create and append a new column "SOP_LOR_AVG" to the view "SLR_Focus".

alter view stu_adm as select sop , lor , (sop+lor)/2 from admission_predict where Research = 1;
...
## Session 2
start transaction;
update bank_customer set telephoone = 189891899 where customer_id = 123002;
...
update bank_account set balance_amount = balance_amount * 1.03;
commit;
...
start transaction;
select * from bank_account where customer_id = 123001 for update;
update bank_account set balance_amount = balance_amount * 1.03 where customer_id = 123001;
commit;

# Session 2
update bank_account set balance_amount = 12345 where customer_id = 123001;

# Session 3
update bank_account set balance_amount = 76543 where customer_id = 123001;