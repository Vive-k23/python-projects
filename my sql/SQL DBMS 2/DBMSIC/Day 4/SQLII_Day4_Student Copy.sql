...
set autocommit=0;
start transaction;
update  account_details set balance=balance-1000 where acc_id=8;

update account_details set balance=balance+1000 where acc_id=1;
commit;
select * from account_details;
...
create view employee_department as select a.employee_id, first_name, last_name, job_id , salary,a.DEPARTMENT_ID, DEPARTMENT_NAME, a.MANAGER_ID, location_id
 from employee_details a join department_details b using(department_id);
...
create view university as select sop, lor from admission_predict where research=1;
select * from university;
# Q.13 Create and append a new column "SOP_LOR_AVG" to the view "SLR_Focus".
create view slr_focus as select `Serial No.`, `gre score` , `toefl score`, `University Rating`, SOP, LOR, cgpa, research , `chance of admit` , sum(sop+lor)/2 as SOP_LOR_AVG
